...
opt = {
   ...
   -- name = 'rain',
   name = 'myRain',              -- name of the experiment, should generally be passed on the command line
   ...
   model_dir = './Trained-Models', -- models are saved here
...
if opt.continue_train == 1 then
   ...
   netG = util.load(paths.concat(opt.model_dir, opt.name, 'latest_net_G.t7'), opt)
   print('loading previously trained netD...')
   netD = util.load(paths.concat(opt.model_dir, opt.name, 'latest_net_D.t7'), opt)
else
...
paths.mkdir(opt.model_dir .. '/' .. opt.name)

-- save opt
file = torch.DiskFile(paths.concat(opt.model_dir, opt.name, 'opt.txt'), 'w')
...
for epoch = 1, opt.niter do
    ...
    for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
        ...
        if counter % opt.save_display_freq == 0 and opt.display then
            ...
            image.save(paths.concat(opt.model_dir,  opt.name , counter .. '_train_res.png'), image_out)
        ...
        if counter % opt.save_latest_freq == 0 then
            ...
            torch.save(paths.concat(opt.model_dir, opt.name, 'latest_net_G.t7'), netG:clearState())
            torch.save(paths.concat(opt.model_dir, opt.name, 'latest_net_D.t7'), netD:clearState())
        end
    ...
    if epoch % opt.save_epoch_freq == 0 then
        torch.save(paths.concat(opt.model_dir, opt.name,  epoch .. '_net_G.t7'), netG:clearState())
        torch.save(paths.concat(opt.model_dir, opt.name, epoch .. '_net_D.t7'), netD:clearState())
    end
